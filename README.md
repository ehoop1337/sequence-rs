...
| setStartFrame | | |  |
| setLoop | | |  |
| setFPS | number | boolean | Изменяет частоту кадров в секунду. Возможно использовать во время рендеринга, без прерывания анимации, последовательность: вызвает метод `stop()`, изменяет значение `fps`, вызвает метод `start()`. Возвращает `true` в случае успуха, иначе `false`. |
| setSizesCanvas | | |  |
| setCurrentFrame | | |  |
| setSizesFrames | | |  |
